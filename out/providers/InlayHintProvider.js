...
exports.LaravelInlayHintProvider = void 0;
...
class LaravelInlayHintProvider {
     ...
     */
    addMigrationHints(lineNum, text, messages, hints) {
        ...
        if (createMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ ${messages.inlay_table_name}`, vscode.InlayHintKind.Parameter);
        ...
        for (const pattern of columnPatterns) {
            ...
            if (match) {
                ...
                const hint = new vscode.InlayHint(pos, `⬆ ${columnName} (${pattern.type})`, vscode.InlayHintKind.Type);
        ...
        if (foreignMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ ${messages.inlay_foreign_key}`, vscode.InlayHintKind.Parameter);
        ...
        if (idMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ BIGINT UNSIGNED (Primary Key)`, vscode.InlayHintKind.Type);
        ...
        if (timestampsMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ created_at + updated_at`, vscode.InlayHintKind.Type);
        ...
        if (softDeletesMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ deleted_at (TIMESTAMP)`, vscode.InlayHintKind.Type);
        ...
        if (rememberTokenMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ remember_token (VARCHAR 100)`, vscode.InlayHintKind.Type);
     ...
     */
    addRouteHints(lineNum, text, messages, hints) {
        // Route methods
        ...
        for (const method of routeMethods) {
            ...
            if (match) {
                const path = match[1];
                const pos = new vscode.Position(lineNum, match.index + match[0].length);
                const hint = new vscode.InlayHint(pos, `⬆ ${method.toUpperCase()} ${path}`, vscode.InlayHintKind.Type);
        ...
        if (nameMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ ${messages.inlay_route_name}`, vscode.InlayHintKind.Parameter);
        ...
        if (resourceMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ CRUD: index, create, store, show, edit, update, destroy`, vscode.InlayHintKind.Type);
        ...
        if (apiResourceMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ API: index, store, show, update, destroy`, vscode.InlayHintKind.Type);
     ...
     */
    addModelHints(lineNum, text, messages, hints) {
        // protected $fillable = [...]
        ...
        if (fillableMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: ${messages.inlay_fillable}`, vscode.InlayHintKind.Type);
        ...
        if (guardedMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Columns protected from mass assignment`, vscode.InlayHintKind.Type);
        ...
        if (castsMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: ${messages.inlay_casts}`, vscode.InlayHintKind.Type);
        ...
        if (hiddenMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Hidden from JSON/array`, vscode.InlayHintKind.Type);
        ...
        for (const rel of relationships) {
            ...
            if (relationMatch) {
                ...
                const hint = new vscode.InlayHint(pos, `:: ${rel.returns}`, vscode.InlayHintKind.Type);
     ...
     */
    addControllerHints(lineNum, text, messages, hints) {
        // $request->validate([...])
        ...
        if (validateMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Validation rules`, vscode.InlayHintKind.Type);
        ...
        if (viewMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ resources/views/${viewPath}`, vscode.InlayHintKind.Type);
        ...
        if (redirectRouteMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `⬆ Redirects to route: ${routeName}`, vscode.InlayHintKind.Type);
        ...
        if (jsonMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: JSON Response`, vscode.InlayHintKind.Type);
        ...
        if (findOrFailMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Returns ${modelName} or 404`, vscode.InlayHintKind.Type);
        ...
        if (createMatch) {
            ...
            const pos = new vscode.Position(lineNum, createMatch.index + createMatch[0].length);
            const hint = new vscode.InlayHint(pos, `:: Creates new ${modelName}`, vscode.InlayHintKind.Type);
        ...
        if (updateMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Updates $${varName}`, vscode.InlayHintKind.Type);
        ...
        if (deleteMatch) {
            ...
            const hint = new vscode.InlayHint(pos, `:: Deletes $${varName}`, vscode.InlayHintKind.Type);
